...
import {
  ...
  type RelativeAeroCalibrationResult,
  type RelativeAeroCalibrationVariable,
  type RelativeAeroCalibrationObjective,
  type RelativeAeroCalibrationConstraint,
  type RelativeAeroCalibrationEvidence,
...
type ExportFormat = "project" | "vertical-review-json" | "staged-review-json" | "run-library-json" | "flight-csv" | "stage-flight-csv" | "stage-sweep-csv" | "stage-flight-comparison-csv" | "relative-aero-calibration-csv" | "separated-body-csv" | "coupled-body-csv" | "flight-path-geojson" | "sweep-csv" | "uncertainty-csv" | "benchmark-csv" | "aero-polar-csv" | "report" | "dxf" | "stl" | "openscad" | "manufacturing-manifest";
...
type RelativeAeroCalibrationPreview = Readonly<{
...
const DEFAULT_RELATIVE_AERO_CALIBRATION_VARIABLES: readonly RelativeAeroCalibrationVariable[] = [
  { key: "wakeHalfAngleDeg", label: "Wake half-angle", minimum: 2, maximum: 18, initial: 8 },
  { key: "wakeRecoveryDistanceBodyDiameters", label: "Wake recovery distance", minimum: 10, maximum: 80, initial: 30 },
  { key: "peakVelocityDeficitFraction", label: "Peak velocity deficit", minimum: 0.1, maximum: 0.65, initial: 0.5 },
  { key: "maximumVelocityDeficitFraction", label: "Maximum velocity deficit", minimum: 0.2, maximum: 0.9, initial: 0.7 },
];

const DEFAULT_RELATIVE_AERO_CALIBRATION_OBJECTIVES: readonly RelativeAeroCalibrationObjective[] = [
  { metricKey: "weightedResidualRmse", label: "Weighted residual RMSE", direction: "minimize", weight: 2 },
  { metricKey: "peakVelocityDeficitRmse", label: "Peak deficit RMSE", direction: "minimize" },
  { metricKey: "exposureCoverageRmse", label: "Exposure coverage RMSE", direction: "minimize" },
];

const DEFAULT_RELATIVE_AERO_CALIBRATION_CONSTRAINTS: readonly RelativeAeroCalibrationConstraint[] = [
  { metricKey: "matchedObservationFraction", label: "Evidence coverage", relation: "greater-than-or-equal", limit: 1 },
  { metricKey: "simulationFailure", label: "Calibration failures", relation: "less-than-or-equal", limit: 0 },
];
...
function createStageFlightPreviewInputs({
  ...
  const stages: RocketStage[] = propulsivePlans.map((stage) => {
    ...
    const createStageMotor = (
      ...
      const nozzleGeometry = stageNozzleGeometry(
        stage,
      ...
      return {
        ...
        ...(nozzleGeometry.length > 1 ? { nozzles: nozzleGeometry } : {}),
  ...
  return {
    ...
    coupledSeparationPulse: coupledSeparationPulseEnabled
      ? {
          ...
          ...(coupledSeparationPulseAngularEnabled
            ? { relativeAngularDeltaOmegaBodyRadS: { x: 0, y: coupledSeparationPulseAngularDeltaRadS, z: 0 } }
            : {}),
...
function createRelativeAeroCalibrationBodies(
  result: StageFlightPreviewResult,
): RelativeAeroInteractionBody[] {
  const retainedTrace = result.trace.map((point) => ({
    timeS: point.timeS,
    // The staged summary trace retains altitude and velocity, not lateral
    // position. Keep this calibration adapter explicit about that bounded
    // vertical reconstruction rather than inventing a horizontal track.
    positionWorldM: { x: 0, y: 0, z: point.altitudeAglM },
    velocityWorldMps: point.velocityWorldMps,
  }));
  const retainedBody: RelativeAeroInteractionBody = {
    id: "retained-vehicle",
    label: "Retained vehicle",
    releaseTimeS: retainedTrace[0]?.timeS ?? 0,
    trace: retainedTrace,
  };
  const coupledBodies = result.coupledMultiBodyFlight?.trajectories ?? [];
  if (coupledBodies.length > 0) {
    return [
      ...(coupledBodies.some((body) => body.id === "retained-vehicle") ? [] : [retainedBody]),
      ...coupledBodies.map((body) => ({
        id: body.id,
        label: body.label,
  ...
  return [
    retainedBody,
    ...result.separatedBodies.map((body, index) => ({
...
export default function Home() {
  ...
  const [relativeAeroEvidence, setRelativeAeroEvidence] = useState<RelativeAeroCalibrationEvidence | null>(null);
  const [relativeAeroEvidenceError, setRelativeAeroEvidenceError] = useState("");
  const [relativeAeroCalibrationRunning, setRelativeAeroCalibrationRunning] = useState(false);
  const [relativeAeroCalibration, setRelativeAeroCalibration] = useState<RelativeAeroCalibrationPreview | null>(null);
  const [relativeAeroCalibrationFingerprint, setRelativeAeroCalibrationFingerprint] = useState<string | null>(null);
  const [relativeAeroCalibrationError, setRelativeAeroCalibrationError] = useState("");
  ...
  const relativeAeroCalibrationBodies = useMemo(
    () => stageFlightResult ? createRelativeAeroCalibrationBodies(stageFlightResult) : [],
    [stageFlightResult],
  );
  const relativeAeroCalibrationInputFingerprint = useMemo(
    () => relativeAeroEvidence
      ? JSON.stringify([simulationFingerprint, relativeAeroEvidence])
      : null,
    [relativeAeroEvidence, simulationFingerprint],
  );
  const relativeAeroCalibrationIsCurrent =
    relativeAeroCalibration !== null &&
    stageFlightIsCurrent &&
    relativeAeroCalibrationInputFingerprint !== null &&
    relativeAeroCalibrationFingerprint === relativeAeroCalibrationInputFingerprint;
    ...
    const stages = vehicleTopology.stages
      ...
      .map((stage) => {
        ...
        return {
          ...
          allowableShearPa: screen?.material.allowableShearPa ?? null,
          connectorEvidence: stage.connectorEvidence ?? null,
  ...
  const invalidateStageCalibration = () => {
    ...
    setRelativeAeroCalibration(null);
    setRelativeAeroCalibrationFingerprint(null);
    setRelativeAeroCalibrationError("");
  };
  const invalidateRelativeAeroCalibration = () => {
    setRelativeAeroCalibration(null);
    setRelativeAeroCalibrationFingerprint(null);
    setRelativeAeroCalibrationError("");
  };
  const importRelativeAeroEvidence = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.currentTarget.files?.[0];
    event.currentTarget.value = "";
    if (!file) return;
    try {
      const evidence = parseRelativeAeroCalibrationCsv(await file.text(), file.name);
      setRelativeAeroEvidence(evidence);
      invalidateRelativeAeroCalibration();
      setRelativeAeroEvidenceError("");
      notify(`Loaded ${evidence.observations.length} relative-flow evidence pair${evidence.observations.length === 1 ? "" : "s"}`);
    } catch (error) {
      setRelativeAeroEvidenceError(error instanceof Error ? error.message : "Unable to import relative-flow evidence CSV.");
    }
  };
  const clearRelativeAeroEvidence = () => {
    setRelativeAeroEvidence(null);
    setRelativeAeroEvidenceError("");
    invalidateRelativeAeroCalibration();
    notify("Relative-flow evidence cleared");
  };
  ...
  const switchLocalProject = (record: LocalProjectRecord, force = false) => {
    if (!force && record.projectId === activeProjectId) {
      setProjectConsoleOpen(false);
      return;
    }
    try {
      if (storageReady && !force) {
  ...
  const openWorkspaceBackupImport = () => {
    workspaceBackupImportInputRef.current?.click();
  };
  const importWorkspaceBackupFile = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.currentTarget.files?.[0];
    event.currentTarget.value = "";
    if (!file) return;
    try {
      if (file.size > 10_000_000) {
        throw new Error("RocketWorks workspace backups must be 10 MB or smaller.");
      }
      if (!storageReady) {
        throw new Error("The browser workspace is still loading; try the restore again in a moment.");
      }
      const imported = parseLocalWorkspaceBackup(await file.text());
      persistCheckpoint(editableInputs, "Checkpoint before workspace backup import");
      const merged = mergeLocalWorkspaceBackup(projectRegistryRef.current, imported);
      projectRegistryRef.current = merged;
      setProjectRegistry(merged);
      window.localStorage.setItem(
        LOCAL_PROJECT_REGISTRY_STORAGE_KEY,
        serializeLocalProjectRegistry(merged),
      );
      const activeRecord = merged.projects.find(
        (record) => record.projectId === merged.activeProjectId,
      );
      if (activeRecord) switchLocalProject(activeRecord, true);
      setProjectRegistryError("");
      notify(`Merged ${imported.registry.projects.length} workspace project${imported.registry.projects.length === 1 ? "" : "s"}`);
    } catch (error) {
      setProjectRegistryError(error instanceof Error ? error.message : "Unable to import the local workspace backup.");
      notify("Workspace backup import failed; the current design remains open");
    }
  };
  ...
  const exportArtifact = (format: ExportFormat) => {
    try {
      ...
      if (format === "project") {
        ...
        content = createKestrelProjectJson({
          ...
          analyses: {
            ...
            stageCalibration: stageCalibration
              ...
              : null,
            relativeAeroCalibration: relativeAeroCalibration
              ? {
                  adapterVersion: relativeAeroCalibration.result.adapterVersion,
                  modelVersion: relativeAeroCalibration.result.modelVersion,
                  validationStatus: relativeAeroCalibration.result.validationStatus,
                  sourceName: relativeAeroCalibration.result.sourceName,
                  observationCount: relativeAeroCalibration.result.observationCount,
                  current: relativeAeroCalibrationIsCurrent,
                  seed: relativeAeroCalibration.result.result.seed,
                  evaluationCount: relativeAeroCalibration.result.result.evaluationCount,
                  recommendedCandidateId: relativeAeroCalibration.result.result.recommendedCandidateId,
                  paretoFront: relativeAeroCalibration.result.result.paretoFront.map(
                    (candidate) => ({
                      id: candidate.id,
                      variables: candidate.variables,
                      metrics: candidate.metrics,
      ...
      } else if (format === "relative-aero-calibration-csv") {
        if (!relativeAeroCalibration || !relativeAeroCalibrationIsCurrent) {
          throw new Error("Run the current relative-flow evidence calibration before exporting its Pareto rows.");
        }
        filename = `${fileStem}-relative-aero-calibration.csv`;
        mediaType = "text/csv;charset=utf-8";
        content = createRelativeAeroCalibrationCsv(relativeAeroCalibration.result);
        ...
        content = createEngineeringReportMarkdown({
          ...
          relativeAeroCalibration: relativeAeroCalibrationIsCurrent ? relativeAeroCalibration.result : null,
          uncertainty,
          landing: landingPrediction,
          structural: structuralScreen,
          stageStructural: stageStructuralReview,
  ...
  const runRelativeAeroCalibration = () => {
    if (!stageFlightResult || !stageFlightIsCurrent) {
      notify("Run the current coupled preview before calibrating relative-flow evidence");
      return;
    }
    if (!relativeAeroEvidence) {
      notify("Import a relative-flow evidence CSV before starting calibration");
      return;
    }
    if (relativeAeroCalibrationBodies.length < 2) {
      notify("The current staged preview does not contain two released-body traces for relative-flow calibration");
      return;
    }
    setRelativeAeroCalibrationRunning(true);
    setRelativeAeroCalibrationError("");
    setView("flight");
    const inputFingerprint = relativeAeroCalibrationInputFingerprint;
    window.setTimeout(() => {
      try {
        if (!inputFingerprint) throw new Error("Relative-flow evidence is not ready for calibration");
        const nextResult = calibrateRelativeAeroInteraction({
          bodies: relativeAeroCalibrationBodies,
          environmentAt: previewEnvironment.at,
          options: {
            enabled: true,
            wakeHalfAngleDeg: relativeAeroWakeHalfAngleDeg,
          ...
          evidence: relativeAeroEvidence,
          seed: "rocketworks-relative-aero-calibration-v1",
          populationSize: 8,
          generations: 1,
          variables: DEFAULT_RELATIVE_AERO_CALIBRATION_VARIABLES,
          objectives: DEFAULT_RELATIVE_AERO_CALIBRATION_OBJECTIVES,
          constraints: DEFAULT_RELATIVE_AERO_CALIBRATION_CONSTRAINTS,
        });
        setRelativeAeroCalibration({ result: nextResult });
        setRelativeAeroCalibrationFingerprint(inputFingerprint);
        notify("Relative-flow evidence calibration complete");
      } catch (error) {
        const message = error instanceof Error ? error.message : "Unable to calibrate relative-flow evidence";
        setRelativeAeroCalibrationError(message);
        notify(message);
      } finally {
        setRelativeAeroCalibrationRunning(false);
      }
    }, 30);
  };
  const applyRelativeAeroCalibrationRecommendation = () => {
    if (!relativeAeroCalibration || !relativeAeroCalibrationIsCurrent) {
      notify("Run the current relative-flow calibration before applying a recommendation");
      return;
    }
    const candidate = relativeAeroCalibration.result.result.paretoFront.find(
      (item) => item.id === relativeAeroCalibration.result.result.recommendedCandidateId,
    );
    if (!candidate) {
      notify("No feasible relative-flow calibration recommendation is available");
      return;
    }
    if (candidate.variables.wakeHalfAngleDeg !== undefined) setRelativeAeroWakeHalfAngleDeg(candidate.variables.wakeHalfAngleDeg);
    if (candidate.variables.wakeRecoveryDistanceBodyDiameters !== undefined) setRelativeAeroWakeRecoveryDistanceBodyDiameters(candidate.variables.wakeRecoveryDistanceBodyDiameters);
    if (candidate.variables.peakVelocityDeficitFraction !== undefined) setRelativeAeroPeakVelocityDeficitFraction(candidate.variables.peakVelocityDeficitFraction);
    if (candidate.variables.maximumVelocityDeficitFraction !== undefined) setRelativeAeroMaximumVelocityDeficitFraction(candidate.variables.maximumVelocityDeficitFraction);
    setRelativeAeroCalibration(null);
    setRelativeAeroCalibrationFingerprint(null);
    markChanged();
    notify("Mapped relative-flow recommendation applied; rerun the coupled preview to review it");
  };
  ...
  return (
    ...
    >
      ...
      />
      <input
        ref={workspaceBackupImportInputRef}
        className="sr-only"
        type="file"
        accept=".json,application/json"
              ...
              <section className="stage-flight-card" aria-labelledby="stage-flight-title">
                ...
                <div className="stage-flight-model-options">
                  ...
                  <div className="field-group">
                    ...
                    <select
                      ...
                      value={!coupledMultiBodyIncludeRetainedBody ? "detached-only" : coupledMultiBodyRetainedBodyMode}
                      onChange={(event) => {
                        ...
                        if (nextMode === "detached-only") {
                          setCoupledSeparationPulseEnabled(false);
                          setCoupledSeparationPulseAngularEnabled(false);
                        }
                  ...
                  <div className="field-group">
                    ...
                    <select
                      ...
                      onChange={(event) => {
                        ...
                        if (!enabled) setCoupledSeparationPulseAngularEnabled(false);
                  ...
                  {coupledSeparationPulseEnabled && (
                    <>
                      ...
                      />
                      <div className="field-group">
                        <label htmlFor="first-separation-angular-mode">Angular separation pulse</label>
                        <select
                          id="first-separation-angular-mode"
                          value={coupledSeparationPulseAngularEnabled ? "enabled" : "disabled"}
                          onChange={(event) => {
                            const enabled = event.target.value === "enabled";
                            setCoupledSeparationPulseAngularEnabled(enabled);
                            if (enabled) setCoupledMultiBodyIncludeRetainedBody(true);
                            markChanged();
                          }}
                        >
                          <option value="disabled">None (baseline)</option>
                          <option value="enabled">Enable bounded angular pulse</option>
                        </select>
                        <small>Optional equal-and-opposite torque target around the retained vehicle&apos;s +Y body axis. Both coupled bodies must have rigid-body inertia properties.</small>
                      </div>
                      {coupledSeparationPulseAngularEnabled && (
                        <NumberField
                          id="first-separation-angular-delta"
                          label="Relative angular Δω (+Y)"
                          value={coupledSeparationPulseAngularDeltaRadS}
                          unit="rad/s"
                          min={0.01}
                          max={10}
                          step={0.01}
                          slider
                          onChange={(value) => {
                            setCoupledSeparationPulseAngularDeltaRadS(value);
                            markChanged();
                          }}
                        />
                      )}
                      ...
                      <p className="field-help">The pulse is an inspectable momentum-closure sensitivity study. Optional angular mode adds a sampled-inertia torque pair; neither mode models pyrotechnics, springs, plume exchange, structural flex, or hardware calibration.</p>
                    </>
                  )}
                  <div className="field-group">
                  ...
                  <div className="field-group">
                    ...
                    <select
                      ...
                      value={coupledRelativeAeroForceFeedbackEnabled ? "enabled" : "disabled"}
                      onChange={(event) => {
                        setCoupledRelativeAeroForceFeedbackEnabled(event.target.value === "enabled");
                        markChanged();
                      }}
                    >
                      <option value="disabled">Post-trace only (baseline)</option>
                      <option value="enabled">Feed wake proxy into coupled forces (analytical)</option>
                    </select>
                    <small>Applies the strongest overlapping source-wake velocity deficit only inside the shared-grid coupled track. It is a sensitivity proxy—not CFD, a validated interference database, or a change to the independent detached-body branches.</small>
                  </div>
                  {(relativeAeroInteractionEnabled || coupledRelativeAeroForceFeedbackEnabled) && (
                    <>
                      <NumberField
                ...
                {stageFlightResult ? (
                  <>
                        ...
                        {stageFlightResult.coupledMultiBodyFlight && (
                          <div className="stage-coupled-multi-body-flight">
                              ...
                              <div>
                                ...
                                <h5>{stageFlightResult.coupledMultiBodyFlight.trajectories.some((trajectory) => trajectory.id === "retained-vehicle") ? "Coupled retained + detached flight" : "Coupled detached-body flight"}</h5>
                                  <p>{stageFlightResult.coupledMultiBodyFlight.trajectories.some((trajectory) => trajectory.id === "retained-vehicle")
                                    ...
                                    ? "Runs the retained vehicle and released bodies together on one mission-time grid. The retained handoff evaluates changing staging mass/inertia, caller-supplied propulsion, active-topology aerodynamics, recovery loads, and later authoritative staging velocity impulses; an optional bounded separation-force pulse is analytical only, while mechanism hardware, plume/contact interaction, and validated interference remain out of scope."
                            ...
                            <div className="stage-coupled-multi-body-flight-grid">
                              ...
                              <div><span>Dynamic mass models</span><strong>{stageFlightResult.coupledMultiBodyFlight.dynamicMassBodyCount}</strong><small>{stageFlightResult.coupledMultiBodyFlight.dynamicMassBodyCount > 0 ? "mass/inertia sampled per substep" : "constant mass properties"}</small></div>
                            ...
                            <div className="stage-coupled-multi-body-flight-aero-summary">
                              <span>Coupled wake force feedback</span>
                              <strong>{stageFlightResult.coupledMultiBodyFlight.relativeAeroForceFeedback.enabled ? "Enabled" : "Post-trace only"}</strong>
                            ...
                            {stageFlightResult.coupledMultiBodyFlight.separationMechanisms.enabled && (
                              <div className="stage-coupled-multi-body-flight-aero-summary">
                                ...
                                <small>{stageFlightResult.coupledMultiBodyFlight.separationMechanisms.activeSampleCount} active samples · {stageFlightResult.coupledMultiBodyFlight.separationMechanisms.maximumForceN === null ? "no force sample" : `${stageFlightResult.coupledMultiBodyFlight.separationMechanisms.maximumForceN.toFixed(1)} N peak`} · {stageFlightResult.coupledMultiBodyFlight.separationMechanisms.maximumTorqueNm === null ? "no torque sample" : `${stageFlightResult.coupledMultiBodyFlight.separationMechanisms.maximumTorqueNm.toFixed(1)} N·m peak`} · bounded momentum preview</small>
                              </div>
                            )}
                    ...
                    />
                    <RelativeAeroCalibrationCard
                      result={relativeAeroCalibration}
                      running={relativeAeroCalibrationRunning}
                      error={relativeAeroCalibrationError}
                      evidence={relativeAeroEvidence}
                      evidenceError={relativeAeroEvidenceError}
                      current={stageFlightIsCurrent}
                      resultCurrent={relativeAeroCalibrationIsCurrent}
                      bodyIds={relativeAeroCalibrationBodies.map((body) => body.id)}
                      onImport={importRelativeAeroEvidence}
                      onClear={clearRelativeAeroEvidence}
                      onRun={runRelativeAeroCalibration}
                      onApply={applyRelativeAeroCalibrationRecommendation}
                      onExport={() => void exportArtifact("relative-aero-calibration-csv")}
                    />
                  </>
                ) : (
            ...
            {stageInterfaceLoadReview.interfaces.length > 0 && (
              <div className={`stage-interface-load-card stage-interface-load-${stageInterfaceLoadReview.overallStatus}`}>
                ...
                <div className="stage-interface-load-summary">
                  ...
                  <span>Shear {stageInterfaceLoadReview.shearStatus === "assessed" ? "assessed" : stageInterfaceLoadReview.shearStatus === "review" ? "review" : "not assessed"}</span>
                   <span>Connectors {stageInterfaceLoadReview.connectorStatus === "assessed" ? "assessed" : stageInterfaceLoadReview.connectorStatus === "review" ? "review" : "not assessed"}</span>
                   <span>Eccentric {stageInterfaceLoadReview.connectorEccentricStatus === "assessed" ? "assessed" : stageInterfaceLoadReview.connectorEccentricStatus === "review" ? "review" : "not assessed"}</span>
                </div>
                    ...
                    <div className={`stage-interface-load-row stage-interface-load-row-${interfaceLoad.status}`} key={interfaceLoad.id}>
                      ...
                      <div>
                        <strong>{interfaceLoad.parentLabel ?? "Missing parent"} → {interfaceLoad.childLabel}</strong>
                         <small>{interfaceLoad.attachment} · {interfaceLoad.axialDemandN === null ? "axial demand unavailable" : `axial ${interfaceLoad.axialDemandN.toFixed(1)} N`} · {interfaceLoad.transverseDemandN === null ? "transverse not assessed" : `transverse ${interfaceLoad.transverseDemandN.toFixed(1)} N`} · {interfaceLoad.resultantDemandN === null ? "resultant not assessed" : `resultant ${interfaceLoad.resultantDemandN.toFixed(1)} N`} · {interfaceLoad.transverseFactorOfSafety === null ? "transverse shear FoS unavailable" : `transverse shear FoS ${interfaceLoad.transverseFactorOfSafety.toFixed(2)}×`} · {interfaceLoad.connectorFactorOfSafety === null ? "connector FoS unavailable" : `connector FoS ${interfaceLoad.connectorFactorOfSafety.toFixed(2)}×`} · {interfaceLoad.detail}</small>
                      </div>
                ...
                {stageInterfaceLoadReview.parallelAudits.length > 0 && (
                  <div className="stage-parallel-load-audit">
                        ...
                        <div className={`stage-interface-load-row stage-parallel-load-row-${audit.status}`} key={`parallel-${audit.id}`}>
                          ...
                          <div>
                            <strong>{audit.parentLabel ?? "Missing parent"} → {audit.childLabel}</strong>
                             <small>
                               {audit.instanceCount} instance{audit.instanceCount === 1 ? "" : "s"} · share {audit.loadShareFraction === null ? "—" : `${(audit.loadShareFraction * 100).toFixed(1)}%`} · axial {audit.perInstanceAxialDemandN === null ? "—" : `${audit.perInstanceAxialDemandN.toFixed(1)} N / instance`} · transverse {audit.perInstanceTransverseDemandN === null ? "—" : `${audit.perInstanceTransverseDemandN.toFixed(1)} N / instance`} · radial thrust {audit.perInstanceRadialThrustN === null ? "—" : `${audit.perInstanceRadialThrustN.toFixed(1)} N`} · radial demand {audit.radialDemandN === null ? "—" : `${audit.radialDemandN.toFixed(1)} N`} · eccentric moment {audit.perInstanceEccentricMomentNm === null ? "—" : `${audit.perInstanceEccentricMomentNm.toFixed(2)} N·m`} · shear FoS {audit.radialFactorOfSafety === null ? "unavailable" : `${audit.radialFactorOfSafety.toFixed(2)}×`} · connector FoS {audit.connectorFactorOfSafety === null ? "unavailable" : `${audit.connectorFactorOfSafety.toFixed(2)}×`} · eccentric connector FoS {audit.connectorEccentricFactorOfSafety === null ? "unavailable" : `${audit.connectorEccentricFactorOfSafety.toFixed(2)}×`}
                            </small>
                          </div>
                    ...
                    <p className="stage-parallel-load-note">Equal-share radial placement is a force-scale audit. Shell-section shear, connector direct shear, and optional fastener-group eccentric reserve are separate screens; symmetric resultant cancellation does not remove local joint loads, bearing, bending, pull-through, preload, fasteners, or transient response.</p>
                  </div>
                )}
                <p className="stage-interface-load-note">{stageInterfaceLoadReview.accelerationBasis === "trace-peak-with-baseline" ? "Current staged trace informs the axial acceleration envelope; the peak-thrust baseline is retained when larger. " : "Bounded common-acceleration screen only. "}{stageInterfaceLoadReview.transverseAccelerationBasis === "trace-body-transverse" ? "Body-transverse force telemetry is shown separately and receives a shell-section shear proxy when material evidence exists. " : "Body-transverse trace telemetry is unavailable. "}Optional connector evidence uses direct single-shear area only; parallel stages can also supply a fastener-group radius for a conservative eccentric moment screen. Bearing, pull-through, preload, prying, fatigue, local contact, transient response, and flight-safety validation remain outside this review.</p>
              </div>
            )}
                ...
                <article className={stage.enabled ? "topology-stage active" : "topology-stage"} key={stage.id}>
                  ...
                  <div className="topology-stage-body">
                    ...
                    <div className="topology-stage-fields">
                      ...
                      {stage.role !== "payload" && <details className="topology-nozzle-editor">
                        <summary>Multi-nozzle layout {(stage.nozzleCount ?? 1) > 1 ? `· ${stage.nozzleCount} fixed nozzles` : "· single nozzle"}</summary>
                        <p>Split each motor's thrust curve across an equal-share fixed nozzle cluster. Positions and outward cant are analytical geometry only; gimbal hardware, manifolds, plume interaction, and nozzle-level failures remain outside scope.</p>
                        <TopologyNumberField id={`${stage.id}-nozzle-count`} label="Nozzles per motor" value={stage.nozzleCount ?? 1} min={1} max={16} step={1} disabled={Boolean(stage.gimbalSchedule?.length)} onChange={(value) => { if (typeof value === "number") updateTopologyStage(stage.id, { nozzleCount: value }); }} />
                        <TopologyNumberField id={`${stage.id}-nozzle-radius`} label="Nozzle radius (m)" value={stage.nozzleRadiusM ?? 0} min={0} max={0.5} step={0.001} disabled={(stage.nozzleCount ?? 1) <= 1} onChange={(value) => { if (typeof value === "number") updateTopologyStage(stage.id, { nozzleRadiusM: value }); }} />
                        <TopologyNumberField id={`${stage.id}-nozzle-cant`} label="Nozzle outward cant (deg)" value={stage.nozzleCantAngleDeg ?? 0} min={0} max={15} step={0.1} disabled={(stage.nozzleCount ?? 1) <= 1} onChange={(value) => { if (typeof value === "number") updateTopologyStage(stage.id, { nozzleCantAngleDeg: value }); }} />
                        <TopologyNumberField id={`${stage.id}-nozzle-azimuth`} label="Nozzle start azimuth (deg)" value={stage.nozzleCantAzimuthDeg ?? 0} min={-180} max={180} step={1} disabled={(stage.nozzleCount ?? 1) <= 1} onChange={(value) => { if (typeof value === "number") updateTopologyStage(stage.id, { nozzleCantAzimuthDeg: value }); }} />
                        {stage.gimbalSchedule && stage.gimbalSchedule.length > 0 && <small className="topology-editor-note">Clear the gimbal schedule before enabling a multi-nozzle layout.</small>}
                      </details>}
                      <details className="topology-connector-editor">
                        <summary>Connector evidence {stage.connectorEvidence ? "· active" : "· not supplied"}</summary>
                         <p>Optional upstream fastener-group evidence for the stage interface. Parallel stages may also supply a bolt-circle radius for the conservative eccentric screen; clear it to leave that channel unassessed. Bearing, pull-through, preload, prying, fatigue, local contact, and joint qualification remain outside scope.</p>
                        <label className="topology-failure-toggle"><input type="checkbox" checked={stage.connectorEvidence !== undefined} onChange={(event) => updateTopologyStage(stage.id, { connectorEvidence: event.target.checked ? { count: 4, diameterM: 0.004, allowableShearPa: 100e6, efficiency: 0.7 } : undefined })} /> Use connector evidence</label>
                        <TopologyNumberField id={`${stage.id}-connector-count`} label="Connector count" value={stage.connectorEvidence?.count ?? 4} min={1} max={256} step={1} disabled={!stage.connectorEvidence} onChange={(value) => { if (typeof value === "number" && stage.connectorEvidence) updateTopologyStage(stage.id, { connectorEvidence: { ...stage.connectorEvidence, count: value } }); }} />
                        <TopologyNumberField id={`${stage.id}-connector-diameter`} label="Connector diameter (mm)" value={(stage.connectorEvidence?.diameterM ?? 0.004) * 1000} min={0.1} max={200} step={0.1} disabled={!stage.connectorEvidence} onChange={(value) => { if (typeof value === "number" && stage.connectorEvidence) updateTopologyStage(stage.id, { connectorEvidence: { ...stage.connectorEvidence, diameterM: value / 1000 } }); }} />
                         <TopologyNumberField id={`${stage.id}-connector-shear`} label="Allowable shear (MPa)" value={(stage.connectorEvidence?.allowableShearPa ?? 100e6) / 1e6} min={0.01} max={2000} step={1} disabled={!stage.connectorEvidence} onChange={(value) => { if (typeof value === "number" && stage.connectorEvidence) updateTopologyStage(stage.id, { connectorEvidence: { ...stage.connectorEvidence, allowableShearPa: value * 1e6 } }); }} />
                         <TopologyNumberField id={`${stage.id}-connector-efficiency`} label="Group efficiency (%)" value={(stage.connectorEvidence?.efficiency ?? 0.7) * 100} min={1} max={100} step={1} disabled={!stage.connectorEvidence} onChange={(value) => { if (typeof value === "number" && stage.connectorEvidence) updateTopologyStage(stage.id, { connectorEvidence: { ...stage.connectorEvidence, efficiency: value / 100 } }); }} />
                         {stage.attachment === "parallel" && <TopologyNumberField id={`${stage.id}-connector-group-radius`} label="Fastener-group radius (mm)" value={stage.connectorEvidence?.groupRadiusM === undefined ? "" : stage.connectorEvidence.groupRadiusM * 1000} placeholder="optional" min={0.1} max={500} step={0.1} disabled={!stage.connectorEvidence} onChange={(value) => { if (stage.connectorEvidence) updateTopologyStage(stage.id, { connectorEvidence: { ...stage.connectorEvidence, ...(typeof value === "number" ? { groupRadiusM: value / 1000 } : { groupRadiusM: undefined }) } }); }} />}
                      </details>
          ...
          >
            ...
            <div className="project-console-actions" aria-label="Project handoff actions">
              ...
              <button className="project-console-action" onClick={openWorkspaceBackupImport} disabled={!storageReady}>
                <span className="export-extension">REST</span>
                <span><strong>Restore workspace backup</strong><small>Merge a validated backup into this browser; matching project IDs are replaced and nothing is silently dropped.</small></span>
                <em>↑</em>
              </button>
            </div>
            <div className="history-notice">
              <span>LOCAL ONLY</span>
              <p>RocketWorks does not claim cloud sync or multi-user collaboration yet. Export a workspace backup for the local project index and merge it back later; use a project JSON file for complete single-project handoff or a share link when the recipient has the referenced local data sources.</p>
            </div>
          </section>
        </div>
      )}
      ...
      {historyOpen && (
        ...
        >
          ...
          >
            ...
            <div className="history-notice">
              <span>LOCAL ONLY</span>
              <p>This history stays on this device and browser profile. It is not cloud sync or collaboration, and clearing site data can erase it. Export a workspace backup for the local project index or a RocketWorks project document for complete portable storage.</p>
            </div>
          </section>
        </div>
      )}
        ...
        >
          ...
          >
            ...
            <div className="export-grid">
              ...
              {relativeAeroCalibration && relativeAeroCalibrationIsCurrent && <button onClick={() => exportArtifact("relative-aero-calibration-csv")}>
                <span className="export-extension">CSV</span>
                <span><strong>Relative-flow calibration</strong><small>Pair-level wake-proxy fit with evidence coverage, residual metrics, seed, and explicit unvalidated limits.</small></span>
                <em>↓</em>
              </button>}
...
function RelativeAeroCalibrationCard({
  result,
  running,
  error,
  evidence,
  ...
  result: RelativeAeroCalibrationPreview | null;
  running: boolean;
  error: string;
  evidence: RelativeAeroCalibrationEvidence | null;
  ...
  const variableLabels: Readonly<Record<string, string>> = {
    wakeHalfAngleDeg: "Wake half-angle",
    wakeRecoveryDistanceBodyDiameters: "Recovery distance",
    peakVelocityDeficitFraction: "Peak deficit",
    maximumVelocityDeficitFraction: "Maximum deficit",
  };
  const formatMetric = (candidate: typeof recommendation, key: string, decimals = 3) => {
    const value = candidate?.metrics[key];
    return typeof value === "number" && Number.isFinite(value) ? value.toFixed(decimals) : "—";
  };
  const formatVariable = (key: string, value: number) => {
    if (key === "wakeHalfAngleDeg") return `${value.toFixed(1)}°`;
    if (key === "wakeRecoveryDistanceBodyDiameters") return `${value.toFixed(1)} body Ø`;
    return `${(value * 100).toFixed(1)}%`;
  };
  const formatVariables = (candidate: typeof recommendation) => {
    if (!candidate) return "";
    return Object.entries(candidate.variables)
      .map(([key, value]) => `${variableLabels[key] ?? key} ${formatVariable(key, value)}`)
      .join(" · ");
  };
  return (
    <section className="optimization-card relative-aero-calibration-card" aria-labelledby="relative-aero-calibration-title">
      <div className="event-card-heading">
        <div>
          <strong id="relative-aero-calibration-title">Relative-flow evidence calibration</strong>
          <span>Bounded pair-level fit against imported wake or CFD evidence</span>
        </div>
        <div className="flight-data-actions">
          <label className="flight-data-import-button">
            <input type="file" accept=".csv,text/csv" onChange={onImport} />
            {evidence ? "Replace evidence CSV" : "Import evidence CSV"}
          </label>
          {evidence && <button type="button" className="flight-data-clear" onClick={onClear}>Clear</button>}
          {result && resultCurrent && <button type="button" onClick={onExport}>Export CSV</button>}
        </div>
      </div>
      {evidenceError && <p className="sweep-error" role="alert">{evidenceError}</p>}
      {error && <p className="sweep-error" role="alert">{error}</p>}
      {evidence && (
        <div className="flight-data-meta">
          <span><strong>{evidence.sourceName}</strong> · {evidence.observations.length} evidence pair{evidence.observations.length === 1 ? "" : "s"}</span>
          <span>{bodyIds.length} staged body trace{bodyIds.length === 1 ? "" : "s"} available</span>
        </div>
      )}
      {!evidence ? (
        <div className="optimization-empty">
          <strong>Import pair-level evidence first</strong>
          <p>Use a strict SI CSV with <code>source_body_id</code>, <code>target_body_id</code>, and at least one of <code>exposure_coverage_fraction</code>, <code>peak_velocity_deficit_fraction</code>, or <code>maximum_estimated_dynamic_pressure_delta_pa</code>.</p>
        </div>
      ) : !current ? (
        <div className="optimization-empty"><strong>Run the current coupled preview first</strong><p>Calibration reuses the staged released-body traces and will not fit against an older result.</p></div>
      ) : bodyIds.length < 2 ? (
        <div className="optimization-empty"><strong>Two staged body traces are required</strong><p>Run a preview with a retained vehicle and at least one released body before comparing directed wake pairs.</p></div>
      ) : result && !resultCurrent ? (
        <div className="stale-result-banner stage-stale-result-banner" role="status"><span>RERUN REQUIRED</span><div><strong>This calibration is from an earlier evidence or design state</strong><p>Rerun after changing the staged preview, environment, or imported evidence before interpreting or exporting the fit.</p></div></div>
      ) : running ? (
        <div className="optimization-loading" aria-live="polite"><Skeleton height={92} borderRadius={5} /><span>Evaluating bounded wake-proxy candidates…</span></div>
      ) : optimization && resultCurrent && recommendation ? (
        <>
          <div className="optimization-summary">
            <div><span>Weighted residual</span><strong>{formatMetric(recommendation, "weightedResidualRmse")} σ</strong></div>
            <div><span>Exposure RMSE</span><strong>{formatMetric(recommendation, "exposureCoverageRmse")} σ</strong></div>
            <div><span>Deficit RMSE</span><strong>{formatMetric(recommendation, "peakVelocityDeficitRmse")} σ</strong></div>
            <div><span>Evidence matched</span><strong>{(Number(recommendation.metrics.matchedObservationFraction ?? 0) * 100).toFixed(0)}%</strong></div>
          </div>
          <div className="optimization-candidates" aria-label="Leading relative-flow calibration candidates">
            {optimization.paretoFront.slice(0, 3).map((candidate, index) => (
              <div className={index === 0 ? "optimization-candidate recommended" : "optimization-candidate"} key={candidate.id}>
                <span>{index === 0 ? "Recommendation" : `Tradeoff ${index + 1}`}</span>
                <strong>{formatMetric(candidate, "weightedResidualRmse")} σ residual</strong>
                <small>{formatVariables(candidate)} · {(Number(candidate.metrics.matchedObservationFraction ?? 0) * 100).toFixed(0)}% matched</small>
              </div>
            ))}
          </div>
          <div className="optimization-actions">
          ...
          <p className="flight-data-note">Mapped recommendation: {formatVariables(recommendation)}. This fit measures agreement with the supplied source only; it does not validate wind-tunnel or CFD data, add forces or moments, or establish flight safety.</p>
        </>
      ) : (
        <div className="optimization-empty"><strong>Fit the wake-proxy factors to this evidence</strong><p>The bounded seeded study varies wake half-angle, recovery distance, and deficit limits without feeding the result back into the trajectory.</p><button type="button" onClick={onRun}>Run calibration</button></div>
      )}
      <div className="optimization-disclaimer"><span>UNVALIDATED CALIBRATION</span><p>{result?.result.warnings[0] ?? "Pair coverage, body IDs, and measured channels determine the residual; agreement is diagnostic only."} No CFD, wind-tunnel, force/moment, contact, or flight-safety claim is made.</p></div>
    </section>
  );
}
