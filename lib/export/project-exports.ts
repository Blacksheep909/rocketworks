  ...
  const lines = [
    ...
    ...(input.stageInterfaceLoads
      ? [
          ...
          "",
          "| Interface | Attachment | Downstream mass | Axial demand | Transverse demand | Resultant demand | Axial capacity | Axial FoS | Shear capacity | Transverse FoS | Status |",
          "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---|",
          ...input.stageInterfaceLoads.interfaces.map(
            (interfaceLoad) =>
              `| ${markdownText(`${interfaceLoad.parentLabel ?? "Missing parent"} → ${interfaceLoad.childLabel}`)} | ${markdownText(interfaceLoad.attachment)} | ${interfaceLoad.downstreamMassKg === null ? "not assessed" : `${formatNumber(interfaceLoad.downstreamMassKg, 3)} kg`} | ${interfaceLoad.axialDemandN === null ? "not assessed" : `${formatNumber(interfaceLoad.axialDemandN, 2)} N`} | ${interfaceLoad.transverseDemandN === null || interfaceLoad.transverseDemandN === undefined ? "not assessed" : `${formatNumber(interfaceLoad.transverseDemandN, 2)} N`} | ${interfaceLoad.resultantDemandN === null || interfaceLoad.resultantDemandN === undefined ? "not assessed" : `${formatNumber(interfaceLoad.resultantDemandN, 2)} N`} | ${interfaceLoad.capacityN === null ? "not assessed" : `${formatNumber(interfaceLoad.capacityN, 2)} N`} | ${interfaceLoad.factorOfSafety === null ? "not assessed" : `${formatNumber(interfaceLoad.factorOfSafety, 2)}×`} | ${interfaceLoad.shearCapacityN === null || interfaceLoad.shearCapacityN === undefined ? "not assessed" : `${formatNumber(interfaceLoad.shearCapacityN, 2)} N`} | ${interfaceLoad.transverseFactorOfSafety === null || interfaceLoad.transverseFactorOfSafety === undefined ? "not assessed" : `${formatNumber(interfaceLoad.transverseFactorOfSafety, 2)}×`} | ${markdownText(interfaceLoad.status)} |`,
          ),
          "",
          ...(input.stageInterfaceLoads.parallelAudits.length > 0
            ? [
                ...
                "",
                "| Parallel path | Instances | Share | Per-instance axial | Per-instance transverse | Per-instance resultant | Radial thrust | Radial demand | Shear capacity | Radial FoS | Eccentric moment | Symmetric resultant | Status |",
                "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|",
                ...input.stageInterfaceLoads.parallelAudits.map(
                  (audit) =>
                    `| ${markdownText(`${audit.parentLabel ?? "Missing parent"} -> ${audit.childLabel}`)} | ${audit.instanceCount} | ${audit.loadShareFraction === null ? "not assessed" : `${formatNumber(audit.loadShareFraction * 100, 1)}%`} | ${audit.perInstanceAxialDemandN === null ? "not assessed" : `${formatNumber(audit.perInstanceAxialDemandN, 2)} N`} | ${audit.perInstanceTransverseDemandN === null || audit.perInstanceTransverseDemandN === undefined ? "not assessed" : `${formatNumber(audit.perInstanceTransverseDemandN, 2)} N`} | ${audit.perInstanceResultantDemandN === null || audit.perInstanceResultantDemandN === undefined ? "not assessed" : `${formatNumber(audit.perInstanceResultantDemandN, 2)} N`} | ${audit.perInstanceRadialThrustN === null ? "not assessed" : `${formatNumber(audit.perInstanceRadialThrustN, 2)} N`} | ${audit.radialDemandN === null || audit.radialDemandN === undefined ? "not assessed" : `${formatNumber(audit.radialDemandN, 2)} N`} | ${audit.shearCapacityN === null || audit.shearCapacityN === undefined ? "not assessed" : `${formatNumber(audit.shearCapacityN, 2)} N`} | ${audit.radialFactorOfSafety === null || audit.radialFactorOfSafety === undefined ? "not assessed" : `${formatNumber(audit.radialFactorOfSafety, 2)}×`} | ${audit.perInstanceEccentricMomentNm === null ? "not assessed" : `${formatNumber(audit.perInstanceEccentricMomentNm, 3)} N-m`} | ${audit.symmetricResultantRadialThrustN === null ? "not assessed" : `${formatNumber(audit.symmetricResultantRadialThrustN, 3)} N`} | ${markdownText(audit.status)} |`,
                ),
                "",
                "> Equal-share radial placement is a force-scale audit with an optional shell-section shear proxy. Symmetric cancellation does not certify the per-instance joint; connector geometry, bending, fasteners, eccentricity, and transient response remain unmodeled.",
                "",
              ]
            : []),
          ...
          "",
          "> This is a first-order axial serial load-path proxy with optional body-transverse trace telemetry, separate shell-section shear proxies, and a bounded equal-share parallel force-scale audit. The shear values are not connector qualification; geometry, fasteners, joint capacity, bending, transient loads, separation impulse, and flight safety remain outside scope.",
          "",
        ]
      : []),
