      ...
      ? [
          ...
          ...(input.stageInterfaceLoads.parallelAudits.length > 0
            ? [
                ...
                "",
                "| Parallel path | Instances | Share | Per-instance axial | Per-instance transverse | Per-instance resultant | Radial thrust | Radial demand | Shear capacity | Radial FoS | Connector capacity | Connector FoS | Eccentric moment | Symmetric resultant | Status |",
                "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|",
                ...input.stageInterfaceLoads.parallelAudits.map(
                  (audit) =>
                    `| ${markdownText(`${audit.parentLabel ?? "Missing parent"} -> ${audit.childLabel}`)} | ${audit.instanceCount} | ${audit.loadShareFraction === null ? "not assessed" : `${formatNumber(audit.loadShareFraction * 100, 1)}%`} | ${audit.perInstanceAxialDemandN === null ? "not assessed" : `${formatNumber(audit.perInstanceAxialDemandN, 2)} N`} | ${audit.perInstanceTransverseDemandN === null || audit.perInstanceTransverseDemandN === undefined ? "not assessed" : `${formatNumber(audit.perInstanceTransverseDemandN, 2)} N`} | ${audit.perInstanceResultantDemandN === null || audit.perInstanceResultantDemandN === undefined ? "not assessed" : `${formatNumber(audit.perInstanceResultantDemandN, 2)} N`} | ${audit.perInstanceRadialThrustN === null ? "not assessed" : `${formatNumber(audit.perInstanceRadialThrustN, 2)} N`} | ${audit.radialDemandN === null || audit.radialDemandN === undefined ? "not assessed" : `${formatNumber(audit.radialDemandN, 2)} N`} | ${audit.shearCapacityN === null || audit.shearCapacityN === undefined ? "not assessed" : `${formatNumber(audit.shearCapacityN, 2)} N`} | ${audit.radialFactorOfSafety === null || audit.radialFactorOfSafety === undefined ? "not assessed" : `${formatNumber(audit.radialFactorOfSafety, 2)}×`} | ${audit.connectorCapacityN === null || audit.connectorCapacityN === undefined ? "not assessed" : `${formatNumber(audit.connectorCapacityN, 2)} N`} | ${audit.connectorFactorOfSafety === null || audit.connectorFactorOfSafety === undefined ? "not assessed" : `${formatNumber(audit.connectorFactorOfSafety, 2)}×`} | ${audit.perInstanceEccentricMomentNm === null ? "not assessed" : `${formatNumber(audit.perInstanceEccentricMomentNm, 3)} N-m`} | ${audit.symmetricResultantRadialThrustN === null ? "not assessed" : `${formatNumber(audit.symmetricResultantRadialThrustN, 3)} N`} | ${markdownText(audit.status)} |`,
                ),
                "",
                "> Equal-share radial placement is a force-scale audit with optional shell-section shear and connector direct-shear evidence. Symmetric cancellation does not certify the per-instance joint; bearing, pull-through, preload, bending, fasteners, eccentricity, fatigue, and transient response remain unmodeled.",
                "",
              ]
            : []),
