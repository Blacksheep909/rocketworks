...
export const KESTREL_EXPORT_MODEL_VERSION = "kestrel-export-0.12.0";
...
/**
 * Inputs for the inspectable part-level manufacturing manifest. The manifest
 * is intentionally derived from the same bounded reference geometry as the
 * DXF/STL/OpenSCAD artifacts; it is not a toleranced drawing or a bill of
 * certified materials.
 */
export type RocketManufacturingManifestInput = Readonly<{
...
type ManufacturingManifestStage = Readonly<{
  id: string;
  name: string;
...
function manufacturingManifestStages(
  geometry: RocketCadGeometry,
): readonly ManufacturingManifestStage[] {
  if (!geometry.stageParts?.length) {
    return [{
      id: "core",
      name: "Core",
  ...
  return geometry.stageParts.map((part) => {
    const match = part.id.match(/^(.*)-instance-(\d+)$/);
    const instanceIndex = match ? Number(match[2]) : 1;
    return {
      id: match?.[1] ?? part.id,
      name: part.name,
      instanceIndex: Number.isInteger(instanceIndex) && instanceIndex > 0 ? instanceIndex : 1,
      axialOffsetM: part.axialOffsetM,
      radialOffsetYM: part.radialOffsetYM,
      radialOffsetZM: part.radialOffsetZM,
      geometry: cadGeometryFromStagePart(geometry.projectName, part),
    };
  });
}

function manufacturingManifestNumber(value: number | null | undefined): string | null {
  return value === null || value === undefined ? null : Number(value.toFixed(6)).toString();
}

/**
 * Creates a deterministic CSV part manifest for CAD/manufacturing handoff.
 *
 * Rows describe the bounded reference primitives (nose, airframe, fin set,
 * and nozzle) for every enabled physical stage instance. Stage offsets and
 * source dimensions are preserved in millimetres so a downstream CAD user can
 * audit the assembly before rebuilding it. The output deliberately carries
 * an engineering-preview warning and never implies tolerances, material
 * certification, structural release, or manufacturing approval.
 */
export function createRocketManufacturingManifestCsv(
  input: RocketManufacturingManifestInput,
): string {
  validateCadGeometry(input.geometry);
  assertIsoDate(input.generatedAtIso, "manufacturing manifest timestamp");
  for (const [label, value] of [
    ["material label", input.materialLabel],
    ["material validation status", input.materialValidationStatus],
  ] as const) {
    if (value !== undefined && !value.trim()) {
      throw new Error(`manufacturing manifest ${label} cannot be empty when supplied`);
    }
  }
  const metadata: readonly [string, string | number][] = [
    ["# rocketworks_manufacturing_manifest", 1],
    ["# export_model_version", KESTREL_EXPORT_MODEL_VERSION],
    ["# validation_status", KESTREL_EXPORT_VALIDATION_STATUS],
    ["# generated_at_iso", input.generatedAtIso],
    ["# project_name", input.geometry.projectName.trim()],
    ["# units", "millimetres"],
    ["# material_label", input.materialLabel?.trim() ?? "not supplied"],
    ["# material_validation_status", input.materialValidationStatus?.trim() ?? "not supplied"],
    [
      "# warning",
      "Reference geometry manifest only; verify dimensions, fits, wall thicknesses, materials, loads, and tolerances independently before manufacturing.",
    ],
  ];
  const headers = [
    "part_id",
  ...
  const rows = manufacturingManifestStages(input.geometry).flatMap((stage) => {
    const part = stage.geometry;
    const overallLengthMm = (part.noseLengthM + part.bodyLengthM) * 1000;
    const bodyLengthMm = part.bodyLengthM * 1000;
    const diameterMm = part.diameterM * 1000;
    const noseLengthMm = part.noseLengthM * 1000;
    const nozzleLengthMm = Math.min(45, bodyLengthMm * 0.08);
    const common = [
      stage.id,
      stage.name,
      stage.instanceIndex,
      1,
      manufacturingManifestNumber(stage.axialOffsetM * 1000),
      manufacturingManifestNumber(stage.radialOffsetYM * 1000),
      manufacturingManifestNumber(stage.radialOffsetZM * 1000),
      manufacturingManifestNumber(overallLengthMm),
      manufacturingManifestNumber(null),
      manufacturingManifestNumber(diameterMm),
      part.noseProfile ?? "ogive",
      part.finCount,
      manufacturingManifestNumber(part.finRootChordM * 1000),
      manufacturingManifestNumber(part.finTipChordM * 1000),
      manufacturingManifestNumber(part.finSweepM * 1000),
      manufacturingManifestNumber(part.finSpanM * 1000),
      manufacturingManifestNumber(part.finThicknessM * 1000),
    ] as const;
    const row = (
      partId: string,
      partKind: string,
      ...
      ...common.slice(0, 4),
      ...common.slice(4, 8),
      manufacturingManifestNumber(partLengthMm),
      ...common.slice(9),
      notes,
    ].map((value) => csvCell(value));
    return [
      row(`${stage.id}-nose`, "nose-cone", noseLengthMm, `Profile: ${part.noseProfile ?? "ogive"}. Reference surface only.`),
      row(`${stage.id}-airframe`, "airframe", bodyLengthMm, "Cylindrical reference shell; wall thickness and joints are not defined here."),
      row(`${stage.id}-fin-set`, "fin-set", part.finRootChordM * 1000, `Nominal set of ${part.finCount} fins; airfoil, fillets, and attachment details are not defined here.`),
      row(`${stage.id}-nozzle`, "nozzle", nozzleLengthMm, "Reference tapered nozzle envelope; motor, throat, insulation, and thermal design are not defined here."),
    ];
  });
  ...
  const lines = [
    ...
    ...(input.stageFlight
      ? [
          ...
          ...(input.stageFlight.gimbalControlAuthority
            ...
            : []),
          ...(input.stageFlight.missionLossBudget
            ? [
                "### Thrust-axis loss accounting",
                "",
                `| Diagnostic | Value |`,
                `|---|---:|`,
                `| Status | ${markdownText(input.stageFlight.missionLossBudget.status)} |`,
                `| Trace samples | ${input.stageFlight.missionLossBudget.sampleCount} |`,
                `| Time span | ${formatNumber(input.stageFlight.missionLossBudget.timeSpanS, 3)} s |`,
                `| Thrust-axis coverage | ${formatNumber(input.stageFlight.missionLossBudget.thrustAxisCoverageFraction * 100, 2)}% |`,
                `| Thrust impulse equivalent | ${input.stageFlight.missionLossBudget.thrustImpulseEquivalentMps === null ? "not assessed" : `${formatNumber(input.stageFlight.missionLossBudget.thrustImpulseEquivalentMps, 3)} m/s`} |`,
                `| Net thrust delta-v magnitude | ${input.stageFlight.missionLossBudget.netThrustDeltaVMagnitudeMps === null ? "not assessed" : `${formatNumber(input.stageFlight.missionLossBudget.netThrustDeltaVMagnitudeMps, 3)} m/s`} |`,
                `| Steering dispersion | ${input.stageFlight.missionLossBudget.steeringDispersionMps === null ? "not assessed" : `${formatNumber(input.stageFlight.missionLossBudget.steeringDispersionMps, 3)} m/s`} |`,
                `| Gravity opposition | ${input.stageFlight.missionLossBudget.gravity === null ? "not assessed" : `${formatNumber(input.stageFlight.missionLossBudget.gravity.opposingMps, 3)} m/s`} |`,
                `| Aerodynamic opposition | ${input.stageFlight.missionLossBudget.aerodynamic === null ? "not assessed" : `${formatNumber(input.stageFlight.missionLossBudget.aerodynamic.opposingMps, 3)} m/s`} |`,
                `| Recovery opposition | ${input.stageFlight.missionLossBudget.recovery === null ? "not assessed" : `${formatNumber(input.stageFlight.missionLossBudget.recovery.opposingMps, 3)} m/s`} |`,
                `| Projected / unprojected events | ${input.stageFlight.missionLossBudget.projectedEventCount} / ${input.stageFlight.missionLossBudget.unprojectedEventCount} |`,
                `| Model | \`${markdownText(input.stageFlight.missionLossBudget.modelVersion)}\` |`,
                "",
                ...input.stageFlight.missionLossBudget.assumptions.map((assumption) => `- ${markdownText(assumption)}`),
                ...input.stageFlight.missionLossBudget.warnings.map((warning) => `- **Loss-screen warning:** ${markdownText(warning)}`),
                "",
                "> This thrust-axis projection screen is not a validated mission delta-v or loss budget, performance certification, or flight-safety result.",
                "",
              ]
            : []),
          ...(input.stageFlight.missionDeltaVBridge
            ? [
                "### Ideal-to-trace delta-v bridge",
                "",
                `| Diagnostic | Value |`,
                `|---|---:|`,
                `| Status | ${markdownText(input.stageFlight.missionDeltaVBridge.status)} |`,
                `| Ideal serial-stack delta-v | ${input.stageFlight.missionDeltaVBridge.idealSerialStackDeltaVMps === null ? "not assessed" : `${formatNumber(input.stageFlight.missionDeltaVBridge.idealSerialStackDeltaVMps, 3)} m/s`} |`,
                `| Trace thrust impulse equivalent | ${input.stageFlight.missionDeltaVBridge.traceThrustImpulseEquivalentMps === null ? "not assessed" : `${formatNumber(input.stageFlight.missionDeltaVBridge.traceThrustImpulseEquivalentMps, 3)} m/s`} |`,
                `| Signed ideal-to-trace gap | ${input.stageFlight.missionDeltaVBridge.idealToTraceGapMps === null ? "not assessed" : `${formatNumber(input.stageFlight.missionDeltaVBridge.idealToTraceGapMps, 3)} m/s`} |`,
                `| Trace-to-ideal fraction | ${input.stageFlight.missionDeltaVBridge.traceToIdealFraction === null ? "not defined" : `${formatNumber(input.stageFlight.missionDeltaVBridge.traceToIdealFraction * 100, 2)}%`} |`,
                `| Ideal-to-net-vector gap | ${input.stageFlight.missionDeltaVBridge.idealToNetThrustGapMps === null ? "not assessed" : `${formatNumber(input.stageFlight.missionDeltaVBridge.idealToNetThrustGapMps, 3)} m/s`} |`,
                `| Thrust-axis coverage | ${formatNumber(input.stageFlight.missionDeltaVBridge.thrustAxisCoverageFraction * 100, 2)}% |`,
                `| Serial / excluded stages | ${input.stageFlight.missionDeltaVBridge.serialStageCount} / ${input.stageFlight.missionDeltaVBridge.excludedStageCount} |`,
                `| Model | \`${markdownText(input.stageFlight.missionDeltaVBridge.modelVersion)}\` |`,
                "",
                ...input.stageFlight.missionDeltaVBridge.assumptions.map((assumption) => `- ${markdownText(assumption)}`),
                ...input.stageFlight.missionDeltaVBridge.warnings.map((warning) => `- **Bridge warning:** ${markdownText(warning)}`),
                "",
                "> This composition-to-trace comparison shares units but is not achieved mission delta-v, a validated loss budget, performance certification, or a flight-safety result.",
                "",
              ]
            : []),
    ...
    ...(input.stageInterfaceLoads
      ? [
          ...
          `- Separate transverse/radial shear review: ${markdownText(input.stageInterfaceLoads.shearStatus ?? "not-assessed")}.`,
          `- Separate connector direct-shear review: ${markdownText(input.stageInterfaceLoads.connectorStatus ?? "not-assessed")}.`,
          `- Separate connector eccentricity review: ${markdownText(input.stageInterfaceLoads.connectorEccentricStatus ?? "not-assessed")}.`,
          ...
          "",
          "| Interface | Attachment | Downstream mass | Axial demand | Transverse demand | Resultant demand | Axial capacity | Axial FoS | Shear capacity | Transverse FoS | Connector capacity | Connector FoS | Status |",
          "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|",
          ...input.stageInterfaceLoads.interfaces.map(
            (interfaceLoad) =>
              `| ${markdownText(`${interfaceLoad.parentLabel ?? "Missing parent"} → ${interfaceLoad.childLabel}`)} | ${markdownText(interfaceLoad.attachment)} | ${interfaceLoad.downstreamMassKg === null ? "not assessed" : `${formatNumber(interfaceLoad.downstreamMassKg, 3)} kg`} | ${interfaceLoad.axialDemandN === null ? "not assessed" : `${formatNumber(interfaceLoad.axialDemandN, 2)} N`} | ${interfaceLoad.transverseDemandN === null || interfaceLoad.transverseDemandN === undefined ? "not assessed" : `${formatNumber(interfaceLoad.transverseDemandN, 2)} N`} | ${interfaceLoad.resultantDemandN === null || interfaceLoad.resultantDemandN === undefined ? "not assessed" : `${formatNumber(interfaceLoad.resultantDemandN, 2)} N`} | ${interfaceLoad.capacityN === null ? "not assessed" : `${formatNumber(interfaceLoad.capacityN, 2)} N`} | ${interfaceLoad.factorOfSafety === null ? "not assessed" : `${formatNumber(interfaceLoad.factorOfSafety, 2)}×`} | ${interfaceLoad.shearCapacityN === null || interfaceLoad.shearCapacityN === undefined ? "not assessed" : `${formatNumber(interfaceLoad.shearCapacityN, 2)} N`} | ${interfaceLoad.transverseFactorOfSafety === null || interfaceLoad.transverseFactorOfSafety === undefined ? "not assessed" : `${formatNumber(interfaceLoad.transverseFactorOfSafety, 2)}×`} | ${interfaceLoad.connectorCapacityN === null || interfaceLoad.connectorCapacityN === undefined ? "not assessed" : `${formatNumber(interfaceLoad.connectorCapacityN, 2)} N`} | ${interfaceLoad.connectorFactorOfSafety === null || interfaceLoad.connectorFactorOfSafety === undefined ? "not assessed" : `${formatNumber(interfaceLoad.connectorFactorOfSafety, 2)}×`} | ${markdownText(interfaceLoad.status)} |`,
          ),
          "",
          ...(input.stageInterfaceLoads.parallelAudits.length > 0
            ? [
                ...
                "",
                "| Parallel path | Instances | Share | Per-instance axial | Per-instance transverse | Per-instance resultant | Radial thrust | Radial demand | Shear capacity | Radial FoS | Connector capacity | Connector FoS | Eccentric connector FoS | Eccentric moment | Symmetric resultant | Status |",
                "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|",
                ...input.stageInterfaceLoads.parallelAudits.map(
                  (audit) =>
                    `| ${markdownText(`${audit.parentLabel ?? "Missing parent"} -> ${audit.childLabel}`)} | ${audit.instanceCount} | ${audit.loadShareFraction === null ? "not assessed" : `${formatNumber(audit.loadShareFraction * 100, 1)}%`} | ${audit.perInstanceAxialDemandN === null ? "not assessed" : `${formatNumber(audit.perInstanceAxialDemandN, 2)} N`} | ${audit.perInstanceTransverseDemandN === null || audit.perInstanceTransverseDemandN === undefined ? "not assessed" : `${formatNumber(audit.perInstanceTransverseDemandN, 2)} N`} | ${audit.perInstanceResultantDemandN === null || audit.perInstanceResultantDemandN === undefined ? "not assessed" : `${formatNumber(audit.perInstanceResultantDemandN, 2)} N`} | ${audit.perInstanceRadialThrustN === null ? "not assessed" : `${formatNumber(audit.perInstanceRadialThrustN, 2)} N`} | ${audit.radialDemandN === null || audit.radialDemandN === undefined ? "not assessed" : `${formatNumber(audit.radialDemandN, 2)} N`} | ${audit.shearCapacityN === null || audit.shearCapacityN === undefined ? "not assessed" : `${formatNumber(audit.shearCapacityN, 2)} N`} | ${audit.radialFactorOfSafety === null || audit.radialFactorOfSafety === undefined ? "not assessed" : `${formatNumber(audit.radialFactorOfSafety, 2)}×`} | ${audit.connectorCapacityN === null || audit.connectorCapacityN === undefined ? "not assessed" : `${formatNumber(audit.connectorCapacityN, 2)} N`} | ${audit.connectorFactorOfSafety === null || audit.connectorFactorOfSafety === undefined ? "not assessed" : `${formatNumber(audit.connectorFactorOfSafety, 2)}×`} | ${audit.connectorEccentricFactorOfSafety === null || audit.connectorEccentricFactorOfSafety === undefined ? "not assessed" : `${formatNumber(audit.connectorEccentricFactorOfSafety, 2)}×`} | ${audit.perInstanceEccentricMomentNm === null ? "not assessed" : `${formatNumber(audit.perInstanceEccentricMomentNm, 3)} N-m`} | ${audit.symmetricResultantRadialThrustN === null ? "not assessed" : `${formatNumber(audit.symmetricResultantRadialThrustN, 3)} N`} | ${markdownText(audit.status)} |`,
                ),
                "",
                "> Equal-share radial placement is a force-scale audit with optional shell-section shear, connector direct-shear, and conservative fastener-group eccentric screens. Symmetric cancellation does not certify the per-instance joint; bearing, pull-through, preload, bending, fasteners, local contact, fatigue, and transient response remain unmodeled.",
                "",
              ]
            : []),
          ...
          "",
          "> This is a first-order axial serial load-path proxy with optional body-transverse trace telemetry, separate shell-section shear proxies, optional child-stage connector direct-shear/eccentric evidence, and a bounded equal-share parallel force-scale audit. These values are not joint qualification; bearing, pull-through, preload, fasteners, bending, fatigue, transient loads, separation impulse, and flight safety remain outside scope.",
          "",
        ]
      : []),
