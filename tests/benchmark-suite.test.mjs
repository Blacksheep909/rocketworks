...
test("physics benchmark suite passes deterministic standards and closed-form fixtures", () => {
  ...
  assert.equal(result.modelVersion, "kestrel-physics-benchmark-suite-0.7.0");
  ...
  assert.equal(result.totalCount, 27);
  ...
  assert.ok(result.cases.some((benchmark) => benchmark.id === "stage-interface-transverse-shear-demand"));
...
test("physics benchmark evidence CSV preserves provenance and deterministic rows", () => {
  ...
  assert.match(csv, /# benchmark_model_version,kestrel-physics-benchmark-suite-0\.7\.0/);
  assert.match(csv, /# validation_status,mathematical-regression-tests-only/);
  assert.match(csv, /# result_status,pass/);
  assert.match(csv, /# passed_count,27/);
