...
test("ships a portable simulation run-library handoff", async () => {
  const source = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  const exportSource = await readFile(new URL("../lib/export/simulation-run-library-exports.ts", import.meta.url), "utf8");
  const docs = await readFile(new URL("../docs/engineering/simulation-run-library-export-0.1.md", import.meta.url), "utf8");
  assert.match(source, /Import simulation run library/);
  assert.match(source, /Export all named vertical and staged review runs/);
  assert.match(source, /parseSimulationRunLibraryExport/);
  assert.match(source, /sourceProjectId !== activeProjectId/);
  assert.match(source, /conflicts with an existing local record/);
  assert.match(exportSource, /SIMULATION_RUN_LIBRARY_EXPORT_SCHEMA/);
  assert.match(exportSource, /SIMULATION_RUN_LIBRARY_EXPORT_REVIEW_BOUNDARY/);
  assert.match(exportSource, /MAX_SIMULATION_RUN_LIBRARY_EXPORT_LENGTH = 4_700_000/);
...
test("ships validated device-local autosave and recoverable project history", async () => {
  ...
  assert.match(page, /Checkpoint comparison/);
  ...
  assert.match(projectDiffExports, /before_configuration_fingerprint/);
...
test("ships an interactive multi-stage, booster, and radial-topology editor", async () => {
  ...
  assert.match(page, /authoritative staging velocity impulses/);
  ...
  assert.match(page, /first-separation-pulse-mode/);
  assert.match(page, /Relative separation Δv/);
  assert.match(page, /first-separation-pulse-offset/);
  assert.match(page, /first-separation-pulse-duration/);
  assert.match(page, /Raised cosine \(smooth\)/);
  assert.match(page, /Separation force pulses/);
  ...
  assert.match(page, /vertical-integration-time-step/);
  assert.match(page, /coupled-integration-time-step/);
  assert.match(page, /Vertical integration step/);
  assert.match(page, /Coupled 6DOF base step/);
  assert.match(page, /verticalIntegrationTimeStepS/);
  assert.match(page, /coupledIntegrationTimeStepS/);
  ...
  assert.match(stageInterfaceLoads, /analytical-axial-transverse-radial-connector-eccentricity-load-path-proxy/);
  ...
  assert.match(stageInterfaceLoads, /connectorEvidence/);
  assert.match(stageInterfaceLoads, /connectorCapacityN/);
  assert.match(stageInterfaceLoads, /connectorEccentricFactorOfSafety/);
  assert.match(page, /eccentric moment screen/);
  assert.match(page, /Connector evidence/);
  assert.match(page, /Fastener-group radius/);
